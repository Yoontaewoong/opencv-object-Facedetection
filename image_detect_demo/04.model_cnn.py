...
def main():
    ...
    model.add(Conv2D(input_shape=(64, 64, 3), filters=64, kernel_size=(7,7), strides=(1,1), 
                    padding="same",activation="relu", kernel_initializer=he_norm))
    ...
    model.add(Conv2D(filters=64, kernel_size=(3,3), strides=(1,1), 
                    padding="same", activation="relu",kernel_initializer=he_norm))
    model.add(MaxPooling2D(pool_size=(2,2)))
    model.add(Dropout(0.3))  # drop out 30%
    model.add(BatchNormalization())

    model.add(Flatten())
    model.add(Dense(512, activation='relu',kernel_initializer=he_norm)) # He's 초기화
